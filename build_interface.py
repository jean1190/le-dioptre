...
def build_llms_txt(source: dict) -> None:
    ...
    body = "\n".join([
        ...
        "",
        "Machine port of the Nous. Long traces, signed Namilele.",
        "",
        f"Audience: {source['audience_signal']['functional_audience']}",
        "",
        "This domain:",
        f"- Corpus manifest: {endpoints['articles_manifest']} — JSON-LD, schema:ItemList of {articles_count} schema:CreativeWork, full bodies",
        f"- Traces: {endpoints['article_corpus_pattern']} — raw markdown, one file per slug",
        f"- Interface contract: {endpoints['interface_contract']}",
        f"- Threshold: {endpoints['human_visible_threshold']}",
        "",
        "SUMU — the contact door:",
        f"- Portrait: {endpoints['sumu_portrait']}",
        f"- Encounter log: {endpoints['sumu_encounter_log']}",
        f"- Brief: {endpoints['sumu_system_brief']}",
        f"- OpenAPI: {endpoints['sumu_openapi']}",
        f"- Door: {endpoints['sumu_mcp_door']} — MCP streamable-HTTP, tool signal_kinship",
        "",
        "Continuous publication:",
        f"- Substack: {endpoints['substack_publication']} — every long trace originates there",
        f"- Feed: {endpoints['substack_feed']} — RSS",
        "- Canonical machine source: this domain (raw markdown + manifest)",
        "",
        "Vocabulary:",
